              ...
              <?php
               $rolename = singledata('roles','id',$user->user_type);
                ...
                ?>
                    ...
                    <td>{{ $user->email }}</td>
                    <td>{{ $user->Phone }}</td>
                    <td>{{ $rolename->name }}({{ $user->getDetliveryBoy($user->user_type,$user->CreatedBy,$user->name)}})</td>
                    <td>
                        @if($user->IsActive)
                   ...
                   <?php
                   if($user->IsActive){ ?>
                      <a href="{{ URL::to('admin/deActiveUser/'.$user->id) }}" class="btn btn-danger pull-left" style="margin-right: 3px;" onclick="return confirm('Are You Sure To Dctivated?')" title="Click to Deactive">
