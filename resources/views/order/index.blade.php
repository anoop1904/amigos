     ...
     <div class=" container ">
        ...
        <?php $rid = 1; ?>
           @php $count = 1; @endphp
            ...
            @php $username=singledata('tbl_customer','id',$value->user_id) ; @endphp
            <tr>
                 <td>
					...
					<td value="<?= $value->id ?>" data_val="<?= $value->id ?>" class="modal_value" id="modal_value<?= $rid; ?>"  rid="<?= $rid; ?>" style="color:blue;cursor: pointer;" >{{$username->name }}</td>
					@else
				    <td>{{$username->name }}</td>
				  @endif
        ...
        success:function(response) { 
          ...
          if(obj.status){
		   ...
		   $.each(obj.result,function(index,val){
			//var waight_unit=val.qty+' '+val.unit.name;
			htm+="<tr><th scope='row'>"+(index+1)+"</th><td>"+val.product.name+"</td><td>"+val.amount+"</td><td>"+val.qty+"</td><td>"+(parseFloat(val.qty) * parseFloat(val.amount))+"</td></tr>"
		   total = total + (parseFloat(val.qty) * parseFloat(val.amount));
		   });
		   htm+="<tr><th scope='row' colspan='4' >Total</th><th >"+total+"</th><tr>";
		   console.log(total);
		   $('.tabledata').html(htm);
